...
mapa_cor <- ggplot(df_cor, aes(x = Var1, y = Var2, fill = Correlacion)) +
  ...
  coord_fixed() +
  labs(x = NULL, y = NULL)  # <- esto quita los títulos de los ejes
...
# Función modificada con control de ángulo y formato decimal
grafico_clm_solo1 <- function(var, titulo = NULL, tilt_x = TRUE, usar_coma = FALSE) {
  p <- df %>%
    count(!!sym(var), clm) %>%
    group_by(!!sym(var)) %>%
    mutate(prop = n / sum(n)) %>%
    ...
    geom_text(aes(label = if (usar_coma) {
      gsub("\\.", ",", sprintf("%.1f%%", prop * 100))
    } else {
      scales::percent(prop, accuracy = 0.1)
    }), 
    vjust = -0.5, size = 3.2, color = "black") +
    scale_y_continuous(
      labels = if (usar_coma) function(x) gsub("\\.", ",", sprintf("%.0f%%", x * 100)) else percent_format(accuracy = 1),
    ...
    theme(
      axis.text.x = element_text(
        angle = if (tilt_x) 25 else 0,
        hjust = if (tilt_x) 1 else 0.5,
        size = 9
      ),
...
# Gráficos ajustados
p1 <- grafico_clm_solo1("gender", "Género", tilt_x = FALSE)
p2 <- grafico_clm_solo1("area", "Área", tilt_x = FALSE)
p3 <- grafico_clm_solo1("veh_body", "Tipo de vehículo")  # mantiene inclinación
p4 <- grafico_clm_solo1("veh_age", "Antigüedad del vehículo", tilt_x = FALSE)
p5 <- grafico_clm_solo1("agecat", "Categoría de edad", tilt_x = FALSE)
p6 <- grafico_clm_solo1("exposure_cat", "Nivel de exposición", usar_coma = TRUE)
